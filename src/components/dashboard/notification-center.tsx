...
export function NotificationCenter() {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => { setOpen(false); setActiveReschedule(null); setActiveResolve(null); }} />
              ...
              {loading ? (
                ...
                reminders.map((r) => (
                  <div key={r.id} className="group relative bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm transition-all hover:shadow-md hover:border-amber-200 dark:hover:border-amber-900/50">
                    ...
                    {/* Meta Removed as we moved it to the stamp */}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        onClick={() => { setActiveReschedule(r.id); setActiveResolve(null); setUpdatedNotes(r.notes); }}
                        className="h-8 text-[10px] font-black uppercase tracking-widest text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/30"
                      >
